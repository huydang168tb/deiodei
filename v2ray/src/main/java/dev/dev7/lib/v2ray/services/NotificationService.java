...
public class NotificationService {
    ...
    public TrafficListener trafficListener =
            new TrafficListener() {
                ...
                public void onTrafficChanged(
                        long uploadSpeed,
                        long downloadSpeed,
                        long uploadedTraffic,
                        long downloadedTraffic) {
                    if (mNotificationManager != null && notifcationBuilder != null) {
                        if (isNotificationOnGoing) {
                            notifcationBuilder.setSubText(
                                    "Traffic ↓"
                                            + Utilities.parseTraffic(
                                                    downloadedTraffic, false, false)
                                            + "  ↑"
                                            + Utilities.parseTraffic(
                                                    uploadedTraffic, false, false));
                            ...
                            mNotificationManager.notify(
                                    NOTIFICATION_ID, notifcationBuilder.build());
                        }
                    }
                }
            };
    ...
    public NotificationService(final Service targetService) {
        ...
        PendingIntent notificationContentPendingIntent =
                PendingIntent.getActivity(
                        targetService, 0, launchIntent, judgeForNotificationFlag());
        ...
        PendingIntent disconnectPendingIntent =
                PendingIntent.getService(
                        targetService, 0, disconnectIntent, judgeForNotificationFlag());
        notifcationBuilder = new NotificationCompat.Builder(targetService, notificationChannelID);
        notifcationBuilder
                .setContentTitle(applicationName + " Connecting...")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            notifcationBuilder.setForegroundServiceBehavior(
        ...
        if (mNotificationManager == null) {
            try {
                mNotificationManager =
                        (NotificationManager)
                                targetService.getSystemService(Context.NOTIFICATION_SERVICE);
            } catch (Exception e) {
                return null;
            }
        }
    ...
    private String createNotificationChannelID(
            final Service targetService, final String Application_name) {
        ...
        Objects.requireNonNull(getNotificationManager(targetService))
